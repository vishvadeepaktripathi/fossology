        ...
        if ((!empty($upload_pk) && $prevupload_pk != $upload_pk) || (empty($upload_pk) && 0 == $single_browse)){
          ...
          if ($allusers > 0){
            ...
            if (!empty($uploadRec['user_fk'])){
              $userName = "&nbsp;&nbsp;&nbsp;(" . htmlentities($this->userDao->getUserName($uploadRec['user_fk']), ENT_QUOTES) . ")";
            }else{
              $userName = "&nbsp;&nbsp;&nbsp;(" . htmlentities($this->userDao->getUserName($job['job']['job_user_fk']), ENT_QUOTES) . ")";
            }
          }
